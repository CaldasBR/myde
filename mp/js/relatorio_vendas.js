$(document).ready(function(){
    ...
    carregar_graficos('graf_indicacoes');
    carregar_graficos('graf_indicacoes_status');
    carregar_graficos('graf_indicacoes_bloqueado');
    carregar_chat();
    carregar_rodape();
});
            ...
            case "carrega_grafs":
                switch(jsonData[i].opt){
                    case "graf_indicacoes":
                        drawChart('graf_1',jsonData[i].msg,'ColumnChart','Quantidade de indicações por origem',true);
                    case "graf_indicacoes_status":
                        drawChart('graf_2',jsonData[i].msg,'ColumnChart','Status indicações',false);
                    break;
                    case "graf_indicacoes_bloqueado":
                        drawChart('graf_4',jsonData[i].msg,'Table','Clientes que bloquearam',false);
                        var titulo = document.createElement('p');
                        titulo.value = "Quem bloqueou";
                        getElementById('graf_4').insertBefore(titulo, getElementById('graf_4').firstChild);
