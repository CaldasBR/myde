            ...
            case  "cadastro":
                switch(jsonData[i].opt) {
                    case "le_cadastro":
                        ...
                        $('select').material_select();
                        if(dados_msg.perfil!='default'){
                            document.getElementById("painel_midia").style.display = 'block';
                        }
                        $("#wait").css("display", "none");
                    break;
                    case "expulsa":
...
function atualiza_cadastro(){
    ...
    requisicaoHTTP("POST",url,parametros,true,"salva_cadastro");
    run_waitMe('#wait-conteudo', 1, 'win8_linear');
}
...
function salvar_foto(){
    ...
    var file_size = $("#arquivo").prop("files")[0].size;
    var form_data = new FormData();
    form_data.append("arquivo", file_data);
    form_data.append("before_size", file_size);
    form_data.append("fn", 'img_perfil');
    run_waitMe('#wait-conteudo', 1, 'progressBar');
    $.ajax({
        type: 'POST',               
        ...
        success: function(jsonData){
            $("#wait").css("display", "none");
            if(jsonData.opt === 'Toast'){
                Materialize.toast(jsonData.msg, 4000);
            }else{
                switch(jsonData.funcao){
                    case 'upload':
                        switch(jsonData.opt){
                            case 'refresh':
                                document.getElementById("img_perfil").src = jsonData.msg;
                                document.getElementById("img_dist_card").src = jsonData.msg;
                            break;
                        }
                        $("#wait").css("display", "none");
                    break;               
